...
const updateReplyCommentController = async (req, res, next) => {
  ...
  try {
    ...
    if (comment.replies[replyIndex].user.toString() !== userId) {
      throw new CustomError("You can only update your comment!", 404);
    }
...
const getCommentByPostController = async (req, res, next) => {
...
const dislikeCommentController = async (req, res, next) => {
  ...
  try {
    ...
    if (!comment.likes.includes(userId)) {
      throw new CustomError("You haven't liked this comment!", 400);
    }
