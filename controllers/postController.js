...
const getPostsController = async (req, res, next) => {
  ...
  try {
    ...
    const usersWhoBlockedCurrent = await User.find({ blockList: userId });
    const userIdsWhoBlockedCurrent = usersWhoBlockedCurrent.map((user) =>
      user._id.toString()
    );
    console.log(userIdsWhoBlockedCurrent);

    const excludedUserIds = [
      ...new Set([...blockedUserIds, ...userIdsWhoBlockedCurrent]),
    ];

    const allPosts = await Post.find({
      $or: [{ user: userId }, { user: { $nin: excludedUserIds } }],
